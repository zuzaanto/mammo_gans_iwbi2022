...
class Discriminator(BaseDiscriminator):
    def __init__(self, ndf: int, nc: int, ngpu: int, leakiness: float = 0.2, bias: bool = False):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(self.nc, self.ndf, 4, stride=2, padding=1, bias=self.bias), 
            ...
            nn.Conv2d(self.ndf * 2, self.ndf * 4, 4, stride=2, padding=1, bias=self.bias),
            nn.BatchNorm2d(self.ndf * 4),
            nn.LeakyReLU(self.leakiness, inplace=True),
            # state size. (ndf*4) x 16 x 16 
            nn.Conv2d(self.ndf * 4, self.ndf * 8, 4, stride=2, padding=1, bias=self.bias),
            nn.BatchNorm2d(self.ndf * 8),
            nn.LeakyReLU(self.leakiness, inplace=True),
            # state size. (ndf*8) x 8 x 8
            nn.Conv2d(self.ndf * 8, self.ndf * 16, 4, stride=2, padding=1, bias=self.bias),
