...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="Directory to save 3 new metadata files.",
    )
...
if __name__ == "__main__":
    ...
    for density in DENSITIES:
        ...
        val_proportion_scaled = args.val_proportion / (1.0 - args.train_proportion)
        val_patients_per_density, test_patients_per_density = split_array_into_folds(
            remaining_patients, val_proportion_scaled
        )
        ...
        for train_patient in train_patients:
            if train_patient not in used_patients:
                train_metadata = train_metadata.append(
                    metadata_per_density[
                        metadata_per_density.patient_id == train_patient
                    ]
                )
        used_patients.extend(train_patients)
        for val_patient in val_patients:
            if val_patient not in used_patients:
                val_metadata = val_metadata.append(
                    metadata_per_density[metadata_per_density.patient_id == val_patient]
                )
        used_patients.extend(val_patients)
        for test_patient in test_patients:
            if test_patient not in used_patients:
                test_metadata = test_metadata.append(
                    metadata_per_density[
                        metadata_per_density.patient_id == test_patient
                    ]
                )
        used_patients.extend(test_patients)

    # Some metapoints may not contain density label - we don't want them in any of the splits
    assert (len(train_metadata) + len(val_metadata) + len(test_metadata)) <= len(
        metadata_df
    )

    print(
        f"Split metadata into {len(train_metadata)}, {len(val_metadata)} and {len(test_metadata)} samples."
    )
    print("Saving..")
    ...
    print(
        f"Saved train, test and val metadata to separate files in {out_dirpath.resolve()}"
    )
