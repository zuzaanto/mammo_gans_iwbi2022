...
class BCDRDataset(BaseDataset):
    ...
    def __init__(
        ...
        if self.config.classify_binary_healthy:
            ...
            if self.config.is_trained_on_masses:
                self.metadata.extend(
                    [
                        metapoint
                        for metapoint in self.metadata_unfiltered
                        if "nodule" in metapoint["roi_type"]
                    ]
                )
                logging.info(
                    f"Appended BCDR Masses to metadata. Metadata size: {len(self.metadata)}"
                )
            ...
            if self.config.is_trained_on_calcifications:
                ...
                logging.info(
                    f"Appended BCDR Calcifications to metadata. Metadata size: {len(self.metadata)}"
                )
            ...
            if self.config.is_trained_on_other_roi_types:
                ...
                logging.info(
                    f"Appended BCDR Other ROI types to metadata. Metadata size: {len(self.metadata)}"
                )
    ...
    def __getitem__(self, idx: int):
        ...
        assert metapoint.get("dataset") in [
            "bcdr",
            "bcdr_only_train",
        ...
        if "healthy" not in metapoint or metapoint.get("healthy", False):
            ...
            image = image[
                x : x + h, y : y + w
            ]  # note that the order of axis in healthy bbox is different, TODO change someday
        ...
        roi_type = (
            metapoint["roi_type"]
            if isinstance(metapoint["roi_type"], str)
            else metapoint["roi_type"][0]
        )
