...
class BaseDataset(Dataset):
    ...
    def get_crops_around_bbox(self, bbox: Tuple[int, int, int, int], margin: int, min_size: int,
                              image_shape: Tuple[int, int], config) -> Tuple[int, int, int, int]:
        x, y, w, h = bbox

        x_p, w_p = self.get_measures_for_crop(x, w, margin, min_size, image_shape[1], config)
        y_p, h_p = self.get_measures_for_crop(y, h, margin, min_size, image_shape[0], config,
                                              w_p)  # second dimension depends on length of first dimension

        return (x_p, y_p, w_p, h_p)

    def get_measures_for_crop(self, coord, length, margin, min_length, image_max, config,
                              length_of_other_dimension=None):  # coordinate, length, margin, minimum length, random translation, random zoom
        if length_of_other_dimension is None:
            ...
            r_zoom = int(np.random.normal(loc=l_new * config.zoom_offset,
                                          scale=l_new * config.zoom_spread))  # zoom amount depends on the current length of the crop
        else:
        ...
        r_transl = min(int(-l_new * config.max_translation_offset), int(np.random.normal(loc=0,
                                                                                         scale=l_new * config.translation_spread)))  # amount depends on the current length of the crop
