...
class InbreastDataset(BaseDataset):
        ...
        if self.config.classify_binary_healthy:
            self.metadata.extend(
                [
                    ...
                    if metapoint["dataset"] == "inbreast"
                ]
            )
            logging.info(
                f"Appended InBreast metadata. Metadata size: {len(self.metadata)}"
            )
        else:
            assert (
                self.config.is_trained_on_masses
                or self.config.is_trained_on_calcifications
                or self.config.is_trained_on_other_roi_types
            ), (
            ...
            if self.config.is_trained_on_masses:
                self.metadata.extend(
                    [
                        metapoint
                        for metapoint in self.metadata_unfiltered
                        if metapoint["roi_type"] == "Mass"
                        and metapoint["dataset"] == "inbreast"
                    ]
                )
                logging.info(
                    f"Appended InBreast Masses to metadata. Metadata size: {len(self.metadata)}"
                )
            ...
            if self.config.is_trained_on_calcifications:
                self.metadata.extend(
                    [
                        metapoint
                        for metapoint in self.metadata_unfiltered
                        if metapoint["roi_type"] == "Calcification"
                        and metapoint["dataset"] == "inbreast"
                    ]
                )
                logging.info(
                    f"Appended InBreast Calcifications to metadata. Metadata size: {len(self.metadata)}"
                )
            ...
            if self.config.is_trained_on_other_roi_types:
                self.metadata.extend(
                    [
                        metapoint
                        for metapoint in self.metadata_unfiltered
                        if metapoint["roi_type"] == "Other"
                        and metapoint["dataset"] == "inbreast"
                    ]
                )
                logging.info(
                    f"Appended InBreast Other ROI types to metadata. Metadata size: {len(self.metadata)}"
                )
    ...
    def __getitem__(self, idx: int):
        ...
        assert (
            metapoint.get("dataset") == "inbreast"
        ), "Dataset name mismatch, you're using a wrong metadata file!"
        ...
        try:
            ...
            logging.warning(f"Error in cv2.resize of image (shape: {image.shape}): {e}")
            return None
