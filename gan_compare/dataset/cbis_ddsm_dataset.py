...
class CBIS_DDSMDataset(BaseDataset):
        ...
        if self.config.classify_binary_healthy:
            self.metadata.extend(
                [
                    ...
                    if metapoint["dataset"] == "cbis-ddsm"
                ]
            )
            logging.info(
                f"Appended CBIS-DDSM metadata. Metadata size: {len(self.metadata)}"
            )
        else:
            assert (
            ...
            if self.config.is_trained_on_masses:
                self.metadata.extend(
                    [
                        ...
                        and metapoint["dataset"] == "cbis-ddsm"
                    ]
                )
                logging.info(
                    f"Appended CBIS-DDSM Masses to metadata. Metadata size: {len(self.metadata)}"
                )
            ...
            if self.config.is_trained_on_calcifications:
                ...
                logging.info(
                    f"Appended CBIS-DDSM Calcifications to metadata. Metadata size: {len(self.metadata)}"
                )
            ...
            if self.config.is_trained_on_other_roi_types:
                ...
                logging.info(
                    f"Appended CBIS-DDSM Other ROI types to metadata. Metadata size: {len(self.metadata)}"
                )
    ...
    def __getitem__(self, idx: int):
        ...
        assert (
            metapoint.get("dataset") == "cbis-ddsm"
