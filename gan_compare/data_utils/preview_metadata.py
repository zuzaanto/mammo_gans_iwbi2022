...
if __name__ == "__main__":
    ...
    for metapoint in metadata:
        ...
        if metapoint['dataset'] in ['cbis-ddsm', 'inbreast']:
            ...
            if 'mask_path' in metapoint:
                if Path(metapoint['mask_path']).is_file():
                    ...
                    mask = cv2.resize(mask,list(ds.pixel_array.shape)[::-1], interpolation=cv2.INTER_NEAREST)
        ...
        ax = plt.subplot(121)
        ax.axis('off')
        fig.suptitle(metapoint['dataset'].upper()+ ' ' +str(metapoint['patient_id']), fontsize=15, fontweight='bold')
        ...
        if toggle:
            ...
            ax.set_title('View: '+str(metapoint['laterality'])+'_'+str(metapoint['view']))
        else:
            display = plt.imshow(image_masked)
            ax.set_title('View: '+str(metapoint['laterality'])+'_'+str(metapoint['view'])+'_MASKED')
